...
export default function Client({
  ...
  const fetchBrands = async (newPage: number) => {
    setLoading(true);
    try {
      const brandsData = await getBrands(newPage, limit);
      setBrands(brandsData.brands);
      setTotalPages(brandsData.totalPages);
      setPage(newPage);
    } catch (error) {
  ...
  if (!brands.length) {
    return (
      <div className='flex justify-center items-center h-screen'>
        <div className='text-lg'>No brands found.</div>
      </div>
    );
  }
